...
tracing_physical <- function(input,bin=1e6,chr.len=200*1e6,cut=5){
  if(!require(zoo))
    require(zoo)
  trace.mat1 <- input[,c("f1","m1")] - input$of1
  num.bin <- chr.len/bin
  ...
  loca <- (sort(unique(index.pos),decreasing = F)*bin+ 1.5*bin)
...
## read header from vcf
#vcf  <- "/Users/yanjunzan/Documents/impute/data/255/17116.merge.chr1.infomtative.f2.test10.recode.vcf.gz"
