...
F_id <- ".*1653.*"
M_id <- ".*2109.*"
vcf.file.f2 <- " ./data/180109.all.223.f2.P60.informative.recode.vcf.gz"
...
dim(genotype.hap1)
write.table(genotype.hap1,file = "~/projects/F2_seq/results/F2_n223.test.count.mat.txt",quote = F,sep = "\t")
geno <- genotype.hap1
upper.cut <- 0.8
lower.cut <- 0.2
...
write.table(test,file = "~/projects/F2_seq/results/F2_n223.test.geno.mat.txt",quote = F,sep = "\t")

test <- read.table("data/",sep = "\t",header = T,stringsAsFactors = F)
